    ...
    methods
        ...
        function pos = getChildsPosition(obj)
            if (length(obj.child)>1)
                ...
                pos = obj.origin + obj.child{1}.getChildsPosition()*obj.getRotM();
        ...
        function numChildParts = numChildParts(obj)
            %% numChildParts returns the number of parts within the calling assembly
            numChildParts=0;
            if isprop(obj,'child')
                % Loop through all childs.
                for c = obj.child
                    ...
                    if ~isa(cc,'Part')
                        % cc is an assembly
                        numChildParts = numChildParts+cc.numChildParts();
                    else
                        % cc is a part.
                        numChildParts = numChildParts+1;
                    end
                end
            end
        end
        function [cog, r, m]= cog(obj)
            %% cog calculates the resulting center of gravity for the assembly
            % cog = (\sum_{i=0}^N \vec{r_i} * m_i ) / \sum_{i=0}^N m_i
            N = obj.numChildParts();
            r = nan(N,3);
            m = nan(N,1);
            if isprop(obj,'child')
                ...
                idx_start = 1;
                for ii=1:length(obj.child)
                    if ~isa(obj.child{ii},'Part')
                        % cc is an assembly
                        idx_end = idx_start + obj.child{ii}.numChildParts() - 1;
                        [~, r(idx_start:idx_end,:), m(idx_start:idx_end,1)] = obj.child{ii}.cog();
                        idx_start = idx_end+1;
                    else
                        % cc is a part
                        idx_end = idx_start;
                        r(idx_start:idx_end,:) = obj.child{ii}.getGlobalCOG();
                        m(idx_start:idx_end,1) = obj.child{ii}.mass;
                        idx_start = idx_end+1;                        
                    end
                end
            end
