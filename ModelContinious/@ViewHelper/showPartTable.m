...
if nargin == 3+1
	...
	outStr = sprintf('%s\n % -50s % 30s % 40s',outStr, 'Assembly/Part Description', 'Position', 'Unbalance');
	...
	outStr = sprintf('%s>> Unbalance of topmost Parent (%s) in planeA @%5.1f mm: %6.3f gmm @ %6.1f DEG\n',outStr, model.description, planeA*1e3, norm(u(1:3))*1e6, cart2pol(u(2), u(3))/pi*180);
	outStr = sprintf('%s>> Unbalance of topmost Parent (%s) in planeB @%5.1f mm: %6.3f gmm @ %6.1f DEG\n',outStr, model.description, planeB*1e3, norm(u(4:6))*1e6, cart2pol(u(5), u(6))/pi*180);
	outStr = sprintf('%s>> ^^ SUMMARY ----------------------------------------------------------------------------------------------------\n',outStr);
elseif nargin == 4+1
	...
	if ~isa(model,'Part')
		...
		outStr =sprintf('%s\n % *s% -*s % 9.2f % 9.2f % 9.2f % 9.2f % 9.0f° % 9.2f % 9.0f°',outStr, 2*indentLevel, '', 50-2*indentLevel, model.description, ...
		posx*1e3, posy*1e3, posz*1e3,...
		norm_ua*1e6, ang_ua,...
		norm_ub*1e6, ang_ub);
	end
